...
int vap_svc_start_stop(vap_svc_t *svc, unsigned int radio_index, bool enable)
{
    ...
    for (i = 0; i < num_of_radios; i++) {
        ...
        for (j = 0; j < vap_map->num_vaps; j++) {
            ...
            // Repair exists before deriving the HAL enable flag. On a factory-reset boot the VAP is
            // enabled in config but its VIF_Config row is missing (exists=false); masking the enable
            // with exists first would drive the VAP down (e.g. 2.4GHz private wl0.1).
